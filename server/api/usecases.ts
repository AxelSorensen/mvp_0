...
function setup(usecase_temp) {
  const llm = new ChatOpenAI({model:'gpt-4o'
  , maxTokens: 2000, temperature: usecase_temp, frequencyPenalty: 0.5, modelKwargs: {
    "response_format": { 
      type: "json_object" 
    }}});
  ...
  const instruction = `You are a business intelligence assistant. Given a description of a idea, find 5 specific usecases in different markets where the idea could provide an edge or improvement over current methods. The format of the output should be json with main key 'usecases' and with title of usecase and description. The description should describe the current painpoints of this usecase and explain exactly how the idea might improve these problems. Be factual and non-obvious. Describe certain limitations of the usecase that call for the implementation of the idea. Use 200 words per usecase. Just plain text, no headers`
...
export default defineEventHandler(async (event) => {
  ...
  const llmChain = setup(body.usecase_temp)
  try {
