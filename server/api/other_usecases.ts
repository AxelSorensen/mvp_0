...
const instruction = `You are a business intelligence assistant. Given a description of a idea, find 5 specific usecases in different markets where the idea could provide an edge or improvement over current methods. The format of the output should be json with main key 'usecases' and with title of usecase and description. The description should describe the current painpoints of this usecase and explain exactly how the idea might improve these problems. Be factual and non-obvious. Describe certain limitations of the usecase that call for the implementation of the idea. Use 200 tokens per usecase. Do not mention usecases that are already in the old_usecases list`



const prompt = ChatPromptTemplate.fromMessages([
  ["system", instruction],
  ["human", "Description: {description}\n\nold_usecases: {usecases}\n\nOutput: "],
]);
